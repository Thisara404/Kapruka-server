...
export class DeactivationService {
  ...
  constructor(
    @InjectModel(DeactivatedUser.name)
    private readonly deactivatedModel: Model<DeactivatedUserDocument>,
  ...
  async runDeactivationCleanup(): Promise<void> {
    const now = new Date();
    try {
      const expiredList = await this.deactivatedModel
        .find({
          deleteAfter: { $lte: now },
        })
        .exec();
